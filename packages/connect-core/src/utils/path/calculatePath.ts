...
/**
 * Calculate all forwarding paths for a transaction to `destination`
 * that invokes `directTransaction`.
 *
 */
async function calculateAllForwardingPaths(
  sender: string,
  directTransaction: Transaction,
  forwardersWithPermission: string[],
  forwarders: string[],
  ...
  console.log("hey", allPathsData)

  // Get a list of all forwarders (excluding the forwarders with direct permission)
  const filterForwarders = forwarders.filter(
    forwarder => !includesAddress(forwardersWithPermission, forwarder)
  )
  console.log(filterForwarders)
  ...
  console.log("All paths", allPathsData);
  return allPathsData;
}
...
/**
 * Calculate the transaction path for a transaction to `destination` with some of it's initial forwarders provided
 * that invokes `methodSignature` with `params`.
 *
 */
export async function calculateCustomTransactionPath(
  sender: string,
  destination: string,
  methodSignature: string,
  params: any[],
  ...
  // Attempt transaction pathing algorithm with forwarders
  const forwarders = apps
    .filter(app => app.isForwarder === true)
    .map(app => app.address)
  
  for (const initForwarder of initForwarders) {
    if(!includesAddress(forwarders, initForwarder)){
      return { path: [] } // initial steps provided are not feasible
    }
  }
  console.log("custom forwarders", initForwarders);
  
  let path = [];
  let count = 0;
  const script = encodeCallScript([directTransaction])
  let newScript = script;
  let prevForwarder = sender;
  let forwardingFeePretransaction;

  for (const initForwarder of initForwarders) {
    console.log(initForwarder);
    if (await canForward(initForwarder, prevForwarder, newScript, provider)) {
      let transaction = createForwarderTransaction(initForwarder, newScript)
      newScript = encodeCallScript([transaction])
      prevForwarder = initForwarder
      path.push(transaction)
      
      if (count == 0){
        forwardingFeePretransaction = await buildForwardingFeePretransaction(
          transaction,
          provider
        )
      }
      ...
      return { path: [] } // initial steps couldn't forward successfully
    }
  }
  console.log("custom shiz", path);
  console.log(count);
  console.log(lastSender);
  
  const nextPath = await calculateTransactionPath(
    lastSender,
    destination,
    methodSignature,
    params,
    apps,
    provider,
    finalForwarderProvided? finalForwarder:'',
  )
  console.log("next path", nextPath)
  for (let tx of nextPath.path){
    path.push(tx);
  }
  return {
...
/**
 * Calculate all the transaction paths for a transaction to `destination`
 * that invokes `methodSignature` with `params`.
 *
 */
export async function calculateAllTransactionPaths(
  sender: string,
  destination: string,
  methodSignature: string,
  params: any[],
  ...
  if (
    ...
    try {
       allPaths.push({ path: [directTransaction] });
    } catch (_) {
      // If the direct transaction fails, we give up as we should have been able to
      // perform the action directly
    }
  ...
  console.log(forwardersWithPermission)

  return calculateAllForwardingPaths(
    sender,
    directTransaction,
    forwardersWithPermission,
    forwarders,
...
/**
 * Check whether a transaction path exists to `destination`
 * that invokes `methodSignature` with `params`, with the apps provided
 *
 */
export async function checkCustomIncludedTxPath(
  sender: string,
  destination: string,
  methodSignature: string,
  params: any[],
  ...
  const newForwarders = givenForwarders.filter(forwarder =>
    includesAddress(forwarders, forwarder)
  )
  if(newForwarders.length != givenForwarders.length){
    return { path: [] }
  }
  ...
  // Find forwarders with permission to perform the action
  forwardersWithPermission = newForwarders.filter(forwarder =>
    includesAddress(allowedEntities, forwarder)
  )
...
// Finds the longest path among all paths
export async function calculateLongestTxPath(
  paths: PathData[]
): Promise<PathData> {
  let length = 0;
  let maxIdx = 0;
  for (let i = 0; i < paths.length; i++){
    if (length <= paths[i].path.length){
      length = paths[i].path.length;
      maxIdx = i;
    }
  }
  return paths[maxIdx];
...
export async function calculateShortestTxPath(
  paths: PathData[]
): Promise<PathData> {
  let length = 1000000;
  let minIdx = 0;
